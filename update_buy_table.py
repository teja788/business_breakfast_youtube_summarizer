...
def write_stock_history(rows: list[dict], path: Path) -> None:
    """Per-stock view: every stock (alphabetical) as a small dated table of comments."""
    ...
    for k in sorted(by_stock, key=lambda x: by_stock[x]["name"].lower()):
        ...
        lines.append("")
        lines.append("| Date | Action | Level | What he said |")
        lines.append("| --- | --- | --- | --- |")
        for r in items:
            lines.append(
                f"| {r['date']} | {_md_cell(r['action'])} | {_md_cell(r['price'])} "
                f"| {_md_cell(r['note'])} |")
        lines.append("")
    path.write_text("\n".join(lines), encoding="utf-8")

...
def write_stock_history(rows: list[dict], path: Path) -> None:
    """Per-stock view: every stock (alphabetical) with each dated comment below it."""
    ...
    lines = [
        ...
        "",
        "_Auto-generated by `update_buy_table.py` from `*.buys.json` sidecars._  ",
        "_Every stock he commented on (alphabetical), with each dated comment below "
        "it. Action = his stance that day; price = as stated then (not a live quote)._",
        "",
        f"_Total stocks: {len(by_stock)}._",
        "",
    ]
    for k in sorted(by_stock, key=lambda x: by_stock[x]["name"].lower()):
        g = by_stock[k]
        items = sorted(g["items"], key=lambda r: r["date"])
        dates = ", ".join(r["date"] for r in items)
        lines.append(f"## {g['name']}")
        lines.append(f"_{len(items)} comment(s): {dates}_")
        lines.append("")
        for r in items:
            price = (r["price"] or "").strip()
            price = f" _(level: {price})_" if price else ""
            note = (r["note"] or "").strip() or "(no note)"
            lines.append(f"- **{r['date']}** — **{r['action']}**{price}: {note}")
        lines.append("")
    path.write_text("\n".join(lines), encoding="utf-8")
...
def rebuild_buy_table(kdir: Path = DEFAULT_DIR) -> int:
    ...
    # Per-stock dated comment history.
    write_stock_history(rows, kdir / "stock_history.md")
